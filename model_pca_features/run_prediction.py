...
try:
    ...
    print(f"Error: {e}\nPlease run train_model.py first in this folder.")
    exit(1)

print("=" * 55)
print("    PARKINSONIAN TURNING PATTERN ANALYSIS (PCA - 4 COMPONENTS)")
print("=" * 55 + "\n")
...
for video in sorted(os.listdir(VIDEO_FOLDER)):
    ...
    # Every Monte Carlo trial must now pass through the PCA transformation
    N_TRIALS   = 200
    JITTER_STD = 0.06
    probs = []
    ...
    for _ in range(N_TRIALS):
        ...
        X_trial_raw   = pd.DataFrame([X_base * jitter], columns=feature_names)
        X_trial_scaled = scaler.transform(X_trial_raw)
